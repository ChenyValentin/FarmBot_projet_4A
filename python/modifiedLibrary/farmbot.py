...
class Farmbot():
    ...
    def save_photo(self):
        ...
        response = requests.get(
            'https://my.farmbot.io/api/images', headers=headers)
        images = response.json()
        most_recent_image_url = images[0]['attachment_url']
        googleapi_response = requests.get(most_recent_image_url)
        img = open('../Photos/'+str(images[0]['id'])+'.jpg', 'wb')
        img.write(googleapi_response.content)
        img.close()
